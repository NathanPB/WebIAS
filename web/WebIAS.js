...
var WebIAS = function (_, Kotlin) {
  ...
  function AssemblyParser$writeToMemory$lambda_0(this$AssemblyParser) {
    return function () {
      ...
      this$AssemblyParser.core.memory.get_11rb$(L0).toString() + '\n';
  ...
  AssemblyParser.prototype.writeToMemory = function () {
    ...
    while (tmp$.hasNext()) {
      ...
      var word = {v: new MemoryWord(NumericUtils$Companion_getInstance().binaryToDecimal_61zpoe$(adr + opcode), 40)};
      if (clockdivisor.v) {
        word.v = new MemoryWord(NumericUtils$Companion_getInstance().binaryToDecimal_61zpoe$(word.v.firstInstruction().binary + this.core.memory.get_11rb$(addr.v).firstInstruction().binary), 40);
      }
  ...
  CPU.prototype.nextSearchCycle = function () {
    if (this.CU.get_11rb$('IBR').isEmpty()) {
      ...
      var word = this.core.memory.get_11rb$(this.CU.get_11rb$('PC').decimal);
      var $receiver_0 = this.CU;
  ...
  function IASCore() {
    ...
    var tmp$;
    tmp$ = until(L0, this.memory.capacity).iterator();
    while (tmp$.hasNext()) {
      var i = tmp$.next();
  ...
  function MemoryWord(decimal, size) {
    ...
    Logger$Companion_getInstance().debug_l9770i$(Kotlin.getKClassFromExpression(this), MemoryWord_init$lambda(this));
  }
  ...
  MemoryWord.prototype.firstAddress = function () {
    return new MemoryWord(NumericUtils$Companion_getInstance().binaryToDecimal_61zpoe$(this.firstInstruction().binary.substring(7, 20)), 8);
  };
  ...
  MemoryWord.prototype.firstInstruction = function () {
    return new MemoryWord(NumericUtils$Companion_getInstance().binaryToDecimal_61zpoe$(this.binary.substring(20, 39)), 20);
  };
  ...
  function MemoryWord_init$lambda(this$MemoryWord) {
    return function () {
      return this$MemoryWord.toString();
    };
  }
  MemoryWord.$metadata$ = {
    kind: Kind_CLASS,
  ...
  function GUIHandler_init$lambda_2(this$GUIHandler) {
    return function (it) {
      (new AssemblyParser(this$GUIHandler.core, GUIHandler$Companion_getInstance().terminal.dom.value)).writeToMemory();
      return Unit;
    };
  }
  ...
  function MemoryTable$update$lambda_1(closure$w, closure$size, this$MemoryTable) {
    return function (it) {
      try {
        var $receiver = this$MemoryTable.data;
        var key = closure$w.key;
        var value = new MemoryWord(closure$w.value.decimal, toInt(closure$size.value));
        $receiver.put_xwzc9p$(key, value);
      }
       catch (e) {
        if (!Kotlin.isType(e, Exception))
  ...
  MemoryTable.prototype.update = function () {
    ...
    while (tmp$_3.hasNext()) {
      ...
      var size = Kotlin.isType(tmp$_5 = document.createElement('input'), HTMLInputElement) ? tmp$_5 : throwCCE();
      ...
      switch (GUIHandler$Companion_getInstance().numBase.value) {
        case 'hex':
          ...
          break;
        case 'dec':
          tmp$_6 = w.value.decimal.toString();
          break;
        case 'bin':
          tmp$_6 = w.value.binary;
          break;
        default:tmp$_6 = '666';
          break;
      }
      ...
      size.addEventListener('change', MemoryTable$update$lambda_1(w, size, this));
      tr.innerHTML = tr.innerHTML + ('<td>' + w.key + '<\/td>' + '\n');
      var td = Kotlin.isType(tmp$_7 = document.createElement('td'), HTMLElement) ? tmp$_7 : throwCCE();
      td.appendChild(inp);
      tr.appendChild(td);
      td = Kotlin.isType(tmp$_8 = document.createElement('td'), HTMLElement) ? tmp$_8 : throwCCE();
      td.appendChild(size);
      tr.appendChild(td);
      el.append(tr);
    }
  ...
  function Logger$Companion() {
    ...
    this.blacklist = [getKClass(NumericUtils), getKClass(AssemblyParser)];
  }
  ...
  NumericUtils$Companion.prototype.binaryToDecimal_61zpoe$ = function (num) {
    ...
    for (var i = 0; i <= tmp$; i++) {
      var tmp$_0;
      var current = reversed(Kotlin.isCharSequence(tmp$_0 = num) ? tmp$_0 : throwCCE()).toString().charCodeAt(i);
      if (current === 49) {
        output.v = output.v.add(Kotlin.Long.fromInt(numberToInt(Math_0.pow(2.0, i))));
      }
    }
